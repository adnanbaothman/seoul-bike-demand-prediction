...
<style>
.stApp {{
    background: radial-gradient(circle at top left, #111827 0, {BG} 36%, #020617 100%);
    color: {TEXT};
}}
.block-container {{
    padding-top: 1.4rem;
...
.hero {{
    ...
    border-radius: 28px;
    background: linear-gradient(135deg, rgba(14,165,233,.28), rgba(79,70,229,.22), rgba(2,6,23,.92));
...
.kpi-label {{ color:#BAE6FD; font-size:.82rem; font-weight:700; margin-bottom:.4rem; }}
.kpi-value {{ color:white; font-size:2rem; font-weight:900; letter-spacing:-.04em; }}
...
@st.cache_data
def prepare_model_data(raw_df: pd.DataFrame):
    df = raw_df.rename(
        columns={
            "rented_bike_count": "RentedBike",
            ...
            "snowfall": "Snowfall",
            "dew_point_temperature": "DewPtemperature",
            "rainfall": "Rainfall",
            "season": "Seasons",
            "holiday": "Holiday",
...
def train_models(X: pd.DataFrame, y: pd.Series):
    ...
    for name, model in models.items():
        ...
        fitted[name] = model
    return fitted, pd.DataFrame(results), list(X.columns)
    ...
    row.update(
        {
            ...
            "Rainfall": rainfall,
            "Snowfall": snowfall,
            "Day_Type": 1 if day_type == "Leisure" else 0,
        }
    )
    for col in [f"Seasons_{season}", f"Holiday_{holiday}", f"Functioning Day_{functioning_day}"]:
        if col in row:
            row[col] = 1
...
models, metrics_df, feature_columns = train_models(X, y)
...
st.sidebar.caption("Seoul Bike Sharing Demand CSV uploaded with the project.")
st.sidebar.markdown("**Production model**  ")
st.sidebar.caption("Random Forest trained using the original notebook logic.")

st.markdown(
    """
<div class="hero">
  <div class="hero-badge">Portfolio ML Product · Seoul Mobility</div>
  <h1>🚲 Seoul Bike Demand Intelligence</h1>
  <p>AI-powered forecasting dashboard for hourly bike rental demand, combining weather, seasonality, and operational context into business-ready insights.</p>
</div>
""",
    unsafe_allow_html=True,
)
...
with c2:
    kpi("Best Notebook R²", f"{best_row['R2']:.3f}", best_row["Model"])
with c3:
...
with tab1:
    ...
    st.caption("Adjust the operating scenario below. Prediction uses Random Forest trained from the uploaded SeoulBikeData.csv.")
    ...
    prediction = float(best_model.predict(model_input)[0])
    ...
    with b:
        ...
        if demand_label == "High":
            msg = "Prepare additional bikes and rebalance before peak time. Consider increasing maintenance readiness and station monitoring."
...
with tab3:
    ...
    st.caption("Metrics are calculated from the original notebook preprocessing and train/test split logic.")
    ...
    importance = pd.DataFrame({"Feature": feature_columns, "Importance": best_model.feature_importances_}).sort_values("Importance", ascending=False).head(10)
    fig_imp = px.bar(
